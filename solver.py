...
class SudokuSolver:
    ...
    def reduce_all_domains(self):
        ...
        *Indication: Vous pouvez utiliser les fonction ``get_row``, ``get_col`` et ``get_region`` de la grille*
        ...
        #crée un dictionnaire ('tableau' de 9x9) dont les éléments sont vides si la case a déjà été remplie ou contenant les possiblités si elles n'ont pas été remplies
        empty_pos = self.sudoku.get_empty_pos()
        for i, j in self.sudoku.grid:
            self.val_pos[i, j] = []
        for i, j in empty_pos:
            val_pos_temp = [1, 2, 3, 4, 5, 6, 7, 8, 9]
            val_pos_temp = [x for x in val_pos_temp if x not in self.sudoku.get_row(i)]
            val_pos_temp = [x for x in val_pos_temp if x not in self.sudoku.get_col(j)]
            val_pos_temp = [x for x in val_pos_temp if x not in self.sudoku.get_region(i // 3, j // 3)]
            self.val_pos[i, j] = val_pos_temp
    ...
    def reduce_domains(self, last_i, last_j, last_v):
        ...
        for n in range(9):
            self.val_pos[last_i, n] = [x for x in self.val_pos[last_i, n] if x is not last_v]
            self.val_pos[n, last_j] = [x for x in self.val_pos[n, last_j] if x is not last_v]
        last_row = 3*(last_i//3)
        last_col = 3*(last_j//3)
        for i in range(3):
            for j in range(3):
                self.val_pos[last_row + i, last_col + j] = [x for x in self.val_pos[last_row + i, last_col + j] if x is not last_v]
    ...
    def commit_one_var(self):
        ...
        for i in range(9):
            ...
            liste_pos_col = []
            for j in range(9):
                #cherche une case avec une seule valeur possible
                if len(self.val_pos[i, j]) == 1:
                    v = self.val_pos[i, j][0]
                    self.sudoku.write(i, j, v)
                    self.val_pos[i, j] = []
                    return i, j, v  
                #détermine toutes les valeurs possibles sur une colonne et une ligne (region not implemented)
                liste_pos_row += self.val_pos[i, j]
                liste_pos_col += self.val_pos[j, i]
            #cherche si une valeur n'est présente qu'une seule fois dans les valeurs possibles d'une ligne ou une colonne
            for a in range(9):
                if liste_pos_row.count(a) == 1:
                    for n in range(9):
                        if a in self.val_pos[i, n]:
                            self.sudoku.write(i, n, a)
                            self.val_pos[i, n] = []
                            return i, n, a
                if liste_pos_col.count(a) == 1:
                    for n in range(9): 
                        if a in self.val_pos[n, i]:
                            self.sudoku.write(n, i, a)
                            self.val_pos[n, i] = []
                            return n, i, a     
        return None
    ...
    def solve_step(self):
        ...
        *Variante avancée: en plus de vérifier s'il ne reste plus qu'une seule possibilité pour une case,
        ...
        commit = self.commit_one_var()
        while commit is not None:
            self.reduce_domains(*commit)
            commit = self.commit_one_var()
    ...
    def branch(self):
        ...
        *Variante avancée: Un choix judicieux de variable libre,
        ...
        for n in range(2, 9):
            for i, j in self.val_pos:
                if len(self.val_pos[i, j]) == n:
                    for m in self.val_pos[i, j]:
                        copy_grid = self.sudoku.copy()
                        copy_grid.write(i, j, m)
                        liste_ss_prob.append(SudokuSolver(copy_grid))
                    return liste_ss_prob
    ...
    def solve(self):
        ...
        if self.is_solved():
            ...
            liste_ss_prob = self.branch()
            if not liste_ss_prob:
                return None
            else:
                for n in liste_ss_prob:
                    retour = n.solve()
